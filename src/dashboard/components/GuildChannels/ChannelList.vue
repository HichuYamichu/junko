<template>
  <v-list rounded subheader style="max-height: 600px" class="overflow-y-auto" v-if="channels">
  ...
  methods: {
    getSectionChannels: function(sectionID = "") {
      return this.channels
        .filter(chan => chan.parentID === sectionID && chan.type !== "category")
        .sort((a, b) => a.rawPosition - b.rawPosition);
    },
    selectChannel: function(id) {
      const variables = { channel: this.channels.find(chan => chan.id === id) };
      const mutation = gql`
        mutation selectChannel($channel: Channel) {
          selectChannel(Channel: $channel) @client
        }
      `;
