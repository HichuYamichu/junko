...
export default class BullyCommand extends Command {
  ...
  public async exec(message: Message, { user, lvl }: { user: User; lvl: number }): Promise<void> {
    if (user.id === message.client.user.id) {
      message.channel.send(`Can't bully me!`, {
    ...
    if (this.client.isOwner(user)) {
      message.util.send(`Can't bully The Creator!`, {
        files: [join(__dirname, '../../../..', 'static/me.gif')]
      });
      return;
    }
