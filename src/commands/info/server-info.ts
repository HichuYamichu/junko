...
export default class ServerInfoCommand extends Command {
  ...
  public async exec(message: Message): Promise<void> {
    ...
    const textChans = message.guild.channels.cache.filter(ch => ch.type === 'text').size;
    const voiceChans = message.guild.channels.cache.filter(ch => ch.type === 'text').size;
    embed
      ...
      .addField(
        'Channels:',
        stripIndents`
        • ${textChans} Text, ${voiceChans} Voice
